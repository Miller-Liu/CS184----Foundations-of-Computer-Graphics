...
/**
 * Part 2:
 * Write your own matrix vector multiplication function. Do not use the built-in
 * CGL function!
 */
Vector3D mult(Matrix3x3 mat, Vector3D input) {
	/* TODO */
	Vector3D temp = Vector3D(0);
	for (int i = 0; i < 3; i++) {
		temp += mat[i] * input[i];
	}
	return temp;
...
class QuadDrawer : public Renderer {
	public:
		QuadDrawer()
				: mat(1. / sqrt(2), 1. / sqrt(2), 0, -1. / sqrt(2), 1. / sqrt(2), 0, 0, 0,
							1),
					a(-.75, .5, 0), b(-.75, -.5, 0.0), c(.75, -.5, 0.0), d(.75, 0.5, 0.0) {}

		~QuadDrawer() {}

		string name() { return "Quad Drawing"; }

		string info() { return "Quad Drawing"; }
		...
		void render() {
			glBindTexture(GL_TEXTURE_2D,
										texture); // Comment this out to disable textures
			glBegin(GL_QUADS);
			...
			/* TODO: change the (0,1) below to (0,.1) to zoom into the texture to see
			* changes. */
			glTexCoord2f(0, 1);
			glVertex3f(b_trans[0], b_trans[1], b_trans[2]);
			/* TODO: change the (1,1) below to (.1,.1) to zoom into the texture to see
			* changes. */
			glTexCoord2f(1, 1);
